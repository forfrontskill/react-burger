    ...
    return (
        <header className={style.AppHeader}>
            <nav className={style.Menu}>
                <div>
                    <MenuButton icon={<BurgerIcon type='primary' />} text={'Конструктор'} type={isConstuctor} onClick={handleForwardMainPage} />
                    <MenuButton icon={<ListIcon type='primary'/>} text={'Лента заказов'} type='inactive' onClick={handleFeedPage} />
                </div>
                <div className={style.Logo} onClick={handleForwardMainPage}>
                    <Logo />
                </div>
                <MenuButton icon={<MenuIcon type='primary'/>} text={'Личный кабинет'} type={isProfile} onClick={handleProfile} />
            </nav>
        </header>
    )
}
